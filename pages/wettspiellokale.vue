  ...
  <div class='prose max-w-none'>
    ...
    <div v-if='!pending && data'>
      <div v-for='location in data'>
        <h4>{{ location.name }}</h4>
        <ul>
          <li><a :href='location.googleMapsAddress' target='_blank'>{{ location.address }}</a></li>
          <li>Kapazität Bühne: {{ location.capacity }}</li>
          <li>Module: {{ location.modules }}</li>
          <li v-if='location.einspiellokal'>
            Einspiellokal: <a :href='location.einspiellokal.googleMapsAddress'
                              target='_blank'>{{ location.einspiellokal.name }}</a>
          </li>
          <li v-if='location.instrumentendepot'>
            Instrumentendepot: <a :href='location.instrumentendepot.googleMapsAddress'
                                  target='_blank'>{{ location.instrumentendepot.name }}</a>
          </li>
          <li v-if='location.juryfeedback'>
            Jury-Besprechnung: <a :href='location.juryfeedback.googleMapsAddress'
                                  target='_blank'>{{ location.juryfeedback.name }}</a>
          </li>
        </ul>
      </div>
      <p>
