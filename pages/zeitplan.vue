  ...
  <div class='prose max-w-none'>
    ...
    <div v-if='!pending && data'>
      ...
      <div v-for='day in data.days'>
        <h3>{{ day }}</h3>
        <div class='relative overflow-x-auto' v-for='location in locations(day)'
             v-if='filterRows(entries(day, location.id)).length > 0'>
          <h4>📌 <a :href='location.googleMapsAddress' target='_blank'>{{ location.name }}</a></h4>
          <table class='table-auto w-full text-sm text-left'>
            ...
            <tbody>
            <tr v-for='(entry, index) in filterRows(entries(day, location.id))' class='border-b'
...
function filterRows(entries: TimetableOverviewEntryDTO[]): TimetableOverviewEntryDTO[] {
  if (search.value && search.value.length > 0) {
    return entries.filter(value => {
      ...
      return value.vereinsname.toLowerCase().includes(searchString) ||
