  ...
  <div class='prose prose-img:rounded-lg prose-a:text-blau'>
    ...
    <div v-if='!pending && data'>
      <div v-for='verein in data'>
        <h4 @click='expand(verein)' :class="{
          'underline': !isExpanded(verein),
          'cursor-pointer': !isExpanded(verein),
        }">{{ verein.name }}</h4>
        <div v-if='isExpanded(verein)'>
          <p v-if='verein.homepage'><a :href='verein.homepage' target='_blank'>Website</a></p>
          <p class='whitespace-pre-line' v-if='verein.websiteText'>{{ verein.websiteText }}</p>
